...
describe("ProducerService", () => {
  ...
  const mockFarmService = {
    getAll: jest.fn(),
    getTotalAreaByProducerId: jest.fn(),
    create: jest.fn(),
    update: jest.fn(),
    delete: jest.fn(),
  };
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: "IFarmService", useValue: mockFarmService },
      ],
    }).compile();
  ...
  describe("getTotalArea", () => {
    it("should return total area for existing producer", async () => {
      mockProducerRepository.getById.mockResolvedValue(genericProducer);
      mockFarmService.getTotalAreaByProducerId.mockResolvedValue(5000);

      const result = await service.getTotalArea("1");

      expect(mockProducerRepository.getById).toHaveBeenCalledWith("1");
      expect(mockFarmService.getTotalAreaByProducerId).toHaveBeenCalledWith(
        "1",
      );
    ...
    it("should return 0 when producer has no farms", async () => {
      mockProducerRepository.getById.mockResolvedValue(genericProducer);
      mockFarmService.getTotalAreaByProducerId.mockResolvedValue(0);
    ...
    it("should throw NotFoundException when producer does not exist", async () => {
      ...
      await expect(service.getTotalArea("999")).rejects.toThrow(
        NotFoundException,
      );
      await expect(service.getTotalArea("999")).rejects.toThrow(
        "Producer not found.",
      ...
      expect(mockFarmService.getTotalAreaByProducerId).not.toHaveBeenCalled();
    });
  });
